 ...
 *
 * Every RLS policy in this schema targets `yiwuflow_app`. PostgreSQL lets
 ...
 * That is exactly what was found in production: `yiwuflow_app` was NOLOGIN, so
...
export const RUNTIME_ROLE = 'yiwuflow_app';
...
export function checkRuntimeRole(
  ...
  if (opts.expectRole && identity.currentUser !== RUNTIME_ROLE) problems.push('wrong_role');
...
export function describeUnsafeRole(v: RoleVerdict): string {
  ...
  return [
    ...
    `  expected:     ${RUNTIME_ROLE} with rolsuper=false, rolbypassrls=false`,
