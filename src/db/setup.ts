...
export async function setupProgress(tx: Tx, businessId: BusinessId): Promise<SetupProgress> {
  ...
  // first_success joins conversations so an approved draft must belong to one.
  const r = (await sql<{
    profile_done: boolean; products_done: boolean; name_done: boolean; channels_done: boolean; first_success_done: boolean;
  }>`
    select
      ...
      exists(select 1 from channels ch
              ...
              where ch.business_id = ${businessId}::uuid and ch.kind = 'whatsapp' and ch.status = 'connected') as channels_done,
  ...
  return setupFrom({
    ...
    channels: r.channels_done, first_success: r.first_success_done,
  });
}
