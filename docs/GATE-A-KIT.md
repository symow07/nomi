...
## 2 · 试点周（7天，一天一个信任节拍）
**这一周的产品侧节拍此前是按 core/trust/journey.ts 写的——那个模块从来没有被
调用过，M34.8 已删除。** 下面只列真正跑得起来的：

- Day1 第一条审批 —— `/app/inbox`，真实存在。
- Day2 第一次修改被记住 —— 改过的草稿会记下来（drafts.status = 'edited'）。
- Day4 第一次抽查 —— **M34.7 起是真的**：她做完的活会攒成抽查，你在 `/app/employee`
  上确认，攒够了才谈晋升。此前抽查没有任何写入方，晋升永远不可能发生。
- Day5 老买家记忆、Day6 进度更新、Day7 员工周报 —— **没有做**。买家记忆和周报
  的模块都只有代码没有入口，已随 M34.8 删除。不要在试点里演示这三样。
