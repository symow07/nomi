...
- **every `CREATE POLICY` statement fails** with `role "yiwuflow_app" does not exist`
...
```bash
# 1. cluster roles (yiwuflow_app and its attributes)
...
psql "postgresql://postgres@<host>/yiwuflow_restored" -c "alter role yiwuflow_app login password '<new>';"
```
...
# the app role exists
psql "$DST" -tAc "select count(*) from pg_roles where rolname='yiwuflow_app';"

# tenant isolation actually denies: no tenant context ⇒ zero rows
psql "postgresql://yiwuflow_app@<host>/yiwuflow_restored" -tAc "select count(*) from clients;"   # must be 0
