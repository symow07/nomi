...
| `SITE_HOSTS` | comma list of host names, when set | Phase 5 — the hosts that are the public site, e.g. `nomidoes.com,www.nomidoes.com`. On one of them `/` is the site (`src/api/web/site.ts`), `/privacy` `/terms` `/data-deletion` are served as everywhere, and `/app…` `/login` `/signup` `/verify` answer 301 to the same path on `PUBLIC_BASE_URL` (which must then be set; without it they are served in place). The host of `PUBLIC_BASE_URL` is ignored if listed. **Unset, no host is the site** and `/` is the door everywhere. The page can be read on any host at `/site` (noindex). DNS steps: `docs/SITE-DNS.md`. |
