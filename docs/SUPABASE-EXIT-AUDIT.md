...
| RLS policies | `migrations/0005` etc.: `current_business_id()` + `set_config('app.business_id', …, true)` + per-table policies for role `yiwuflow_app`, which migration 0026 renames to `nomi_app` | **yes — pure PostgreSQL** | kept as-is; this is the tenant-isolation mechanism and it is portable (verified live on PG16) |
